...
class PathFollower(Node):
    ...
    def control_robot(self):
        ...
        # Debugging print for PID output (angular velocity)
            ...
            V_L = linear_velocity - (angular_velocity * 0.189)  # Differential steering for left wheel
            V_R = linear_velocity + (angular_velocity * 0.189)  # Differential steering for right wheel
            V_L = round(V_L, 3)
            V_R = round(V_R, 3)

        # Print the velocities to the console for debugging
            print(f"Sending V_L: {V_L}, V_R: {V_R}")
            self.arduino.write(f"{V_L},{V_R}\n".encode('utf-8'))
